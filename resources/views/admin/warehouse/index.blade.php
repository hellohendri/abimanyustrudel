...
@endsection
@section('content')
<div class="card warehouse_index admikoIndex">
    <div class="card-body">
        <div class="tableBox" id="tableBox">
            ...
            <div class="tableLayout pb-2">
                    ...
                    <thead>
                        <tr data-sort-method='thead'>
							...
							<th scope="col" class="text-nowrap d-none d-sm-table-cell">Outlet</th>
							<th scope="col" class="text-nowrap d-none d-md-table-cell">Jumlah Stock</th>
                            ...
                            @if(Gate::allows('warehouse_allow'))
                    ...
                    @foreach($tableData as $data)
                        <tr>
							<td class="w-5"><a href="{{route("admin.warehouse.edit",[$data->id])}}">{{$data->id}}</a></td>
							<td class="text-nowrap">{{$data->nama_id->nama??""}}</td>
							<td class="text-nowrap d-none d-sm-table-cell">{{$data->outlet_id->nama_outlet??""}}</td>
							<td class="text-nowrap d-none d-md-table-cell">{{$data->jumlah_stock}}</td>
							<td class="d-none d-lg-table-cell">{{$data->tanggal_produksi}}</td>
							<td class="d-none d-lg-table-cell">{{$data->tanggal_expired}}</td>
                            <td class="w-5 no-sort"><a href="{{route("admin.warehouse.edit",[$data->id])}}"><i class="fas fa-edit fa-fw"></i></a></td>
                            @if(Gate::allows(['warehouse_allow']))
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-2">
                    @if(Gate::any(['warehouse_allow']))
                        <a href="{{route('admin.warehouse.create')}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('admiko.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="post" class="w-100" action="{{route("admin.warehouse.delete")}}">
            @method('DELETE')
            @csrf
