...
@section('breadcrumbs')
<li class="breadcrumb-item active" aria-current="page">Fixed Cost</li>
@endsection
@section('pageTitle')
<h1>Fixed Cost</h1>
@endsection
...
@endsection
@section('content')
<div class="card fixed_cost_index admikoIndex">
    <div class="card-body">
        <div class="tableBox" id="tableBox">
            ...
            <div class="tableLayout pb-2">
                    ...
                    <thead>
                        <tr data-sort-method='thead'>
							...
							<th scope="col" class="d-none d-sm-table-cell">Biaya</th>
                            ...
                            @if(Gate::allows('fixed_cost_allow'))
                    ...
                    @foreach($tableData as $data)
                        <tr>
							<td class="w-5"><a href="{{route("admin.fixed_cost.edit",[$data->id])}}">{{$data->id}}</a></td>
							<td class="text-nowrap">{{$data->nama}}</td>
							<td class="d-none d-sm-table-cell"> {{$data->biaya}}</td>
                            ...
                            <td class="w-5 no-sort"><a href="{{route("admin.fixed_cost.edit",[$data->id])}}"><i class="fas fa-edit fa-fw"></i></a></td>
                            @if(Gate::allows(['fixed_cost_allow']))
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-2">
                    @if(Gate::any(['fixed_cost_allow']))
                        <a href="{{route('admin.fixed_cost.create')}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('admiko.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="post" class="w-100" action="{{route("admin.fixed_cost.delete")}}">
            @method('DELETE')
            @csrf
